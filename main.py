...
def handle_message(update: Update, context):
    ...
    if text == "❓ FAQ":
        ...
        update.message.reply_text("Выберите сеть для проверки баланса: 🔍", reply_markup=NETWORK_KEYBOARD)
        context.user_data['action'] = 'check'
        return SELECT_NETWORK_CHECK
    elif text == "➕ Добавить кошелек":
        update.message.reply_text("Выберите сеть для добавления кошелька на мониторинг: ➕", reply_markup=NETWORK_KEYBOARD)
        context.user_data['action'] = 'add'
        return SELECT_NETWORK_ADD
    elif text == "📋 Мои кошельки":
        if not user_id:
            update.message.reply_text("Ошибка: Не удалось определить пользователя. Попробуйте /start. ❌")
            return None
        try:
            wallets = get_user_wallets(user_id)
            if not wallets:
                update.message.reply_text("У вас нет добавленных кошельков. ➕ Добавьте первый!")
            else:
                wallet_list = "\n".join([f"🏷️ {label}: {wallet} ({network})" for wallet, network, _, label in wallets])
                update.message.reply_text(f"📋 Ваши кошельки:\n{wallet_list}")
        except Exception as e:
            logging.error(f"Ошибка при получении кошельков для user_id {user_id}: {e}")
            update.message.reply_text("Ошибка при загрузке кошельков. Попробуйте позже или /start. ❌")
        return None
    elif text == "🔗 TRC20":
        context.user_data['network'] = 'TRC20'
        if context.user_data.get('action') == 'check':
            update.message.reply_text("Введите адрес кошелька для проверки баланса и аналитики: 🔍")
            return ENTER_WALLET_CHECK
        elif context.user_data.get('action') == 'add':
            update.message.reply_text("Введите метку для кошелька (например, 'Мой личный' или оставьте пустым): 🏷️")
        ...
        if 'network' in context.user_data:
            ...
            if context.user_data.get('action') == 'check':
                ...
                if network == 'TRC20' and not wallet.startswith('T'):
                    update.message.reply_text("Ошибка: Адрес TRC20 должен начинаться с 'T'. Попробуйте снова. ❌")
                    return ENTER_WALLET_CHECK
                if network == 'ERC20' and not wallet.startswith('0x'):
                    update.message.reply_text("Ошибка: Адрес ERC20 должен начинаться с '0x'. Попробуйте снова. ❌")
                    return ENTER_WALLET_CHECK
                ...
                if 'label' not in context.user_data:
                    ...
                    update.message.reply_text("Введите адрес кошелька: ➕")
                    ...
                    try:
                        ...
                        update.message.reply_text(f"Кошелек {wallet} ({network}) с меткой '{label}' добавлен для мониторинга. 🔔 Вы получите уведомление при увеличении баланса на 1500+ USDT.")
                    except Exception as e:
                        logging.error(f"Ошибка при добавлении кошелька для user_id {user_id}: {e}")
                        update.message.reply_text("Ошибка при добавлении кошелька. Попробуйте позже или /start. ❌")
                    return ConversationHandler.END

    # Если ничего не подошло, показать главное меню
    update.message.reply_text("Не понял команду. Выберите действие:", reply_markup=MAIN_KEYBOARD)
    return None
